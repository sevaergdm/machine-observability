...
func TestFSEntryParquetRoundTrip(t *testing.T) {
	entries := []FSEntry{entryFSA, entryFSB, entryFSC}
	...
	writer := parquet.NewGenericWriter[FSEntry](f)
	if _, err := writer.Write(entries); err != nil {
		t.Fatalf("error writing parquet file: %v", err)
	}
	...
	got, err := parquet.ReadFile[FSEntry](path)
	if err != nil {
		t.Fatalf("error reading parquet file: %v", err)
	}
